<template>
    <div class="container-fluid">
        ...
        >
            <wj-flex-grid-filter :filterColumns="['Id','CustomId','ProductId','Qty','Status','Address',]" />

            <wj-flex-grid-column binding="Id" header="Id" width="2*" :isReadOnly="true" />
            <wj-flex-grid-column binding="CustomId" header="CustomId" width="2*" :isReadOnly="true" />
            ...
            <wj-flex-grid-column binding="Address" header="Address" width="2*" :isReadOnly="true" />
        </wj-flex-grid>
    </div>
</template>
        ...
        getData: function() {
            ...
            for (let i = 0; i < countries.length; i++) {
                data.push({
                    id: i,
                    Id: countries[i].id,
                    CustomId: countries[i].customId,
                    ProductId: countries[i].productId,
                    Qty: countries[i].qty,
                    Status: countries[i].status,
                    Address: countries[i].address,
                });
            }
            return data;
        },
